...
// Complete question data with actual questions and answers
...
⑤ EAI에서 동기 방식 호출 시 타임아웃이 발생하면 자동으로 비동기 방식으로 전환된다.`,
    ...
    keywords: ['EAI', 'Data 동기화', '트랜잭션', 'SAGA', '2PC']
  },
...
⑤ CompletableFuture와 Reactor는 동일한 개념이다.`,
    ...
    keywords: ['Spring WebFlux', 'Reactive', 'Flux', 'Mono', '비동기']
  },
...
⑤ 트래픽 라우팅 규칙을 통해 카나리 배포가 가능하다.`,
    ...
    keywords: ['Istio', 'Service Mesh', 'Envoy', 'mTLS', 'Circuit Breaker']
  },
...
⑤ initialSize가 minIdle보다 작게 설정되어 있다.`,
    ...
    keywords: ['DBCP', 'Tomcat', 'Connection Pool', 'Performance']
  },
...
⑤ -Xms와 -Xmx 옵션이 더 정확한 메모리 제어를 제공한다.`,
    ...
    keywords: ['JVM', 'Kubernetes', 'Memory', 'Container', 'Heap']
  },
...
⑤ ReferenceError 발생`,
    ...
    keywords: ['JavaScript', 'Scope', 'Hoisting', 'Closure']
  },
...
⑤ 모든 컨테이너는 동일한 QoS 클래스를 가져야 한다.`,
    ...
    keywords: ['Kubernetes', 'QoS', 'Pod', 'Resources']
  },
...
⑤ 환경별(dev, prod) 설정 파일을 분리하여 관리할 수 있다.`,
    ...
    keywords: ['Spring Cloud', 'Config Server', 'Microservices']
  },
...
⑤ DDoS 공격 완화 기능을 제공할 수 있다.`,
    ...
    keywords: ['CDN', 'Cache', 'Edge Server', 'Performance']
  },
...
⑤ Stream 우선순위를 설정하여 중요한 리소스를 먼저 전송할 수 있다.`,
    ...
    keywords: ['HTTP/2', 'Multiplexing', 'HPACK', 'Binary Protocol']
  },
...
⑤ 이벤트 기반 아키텍처와 잘 결합된다.`,
    ...
    keywords: ['SAGA', '분산 트랜잭션', 'MSA', 'Choreography', 'Orchestration']
  },
...
⑤ 토큰에는 사용자 식별 정보만 포함하고 권한 정보는 서버에서 조회한다.`,
    ...
    keywords: ['JWT', 'Security', 'Token', 'Authentication']
  },
...
⑤ Metaspace 사용률이 약 94%로 높아 Class 로딩 관련 메모리 부족이 우려된다.`,
    ...
    keywords: ['JVM', 'GC', 'jstat', 'Monitoring']
  },
...
⑤ JVM 옵션: -Xms1g -Xmx1g -XX:+UseContainerSupport`,
    ...
    keywords: ['JVM', 'Kubernetes', 'Container', 'Memory']
  },
...
⑤ 스레드 풀의 스레드들이 종료되지 않아 메모리가 해제되지 않고 있다.`,
  ...
  // 예상문제 #3 - Kafka/Transactional Outbox Pattern
  {
    id: 'q3-1',
    number: '3-1',
    module: 'Software Architecture 설계/구축',
...
⑤ CDC 프로그램의 메모리 부족`,
    ...
    keywords: ['Transactional Outbox', 'Kafka', 'CDC', '분산 시스템']
  },
...
⑤ 타임스탬프를 파티션 키로 사용하여 시간 순서를 보장한다`,
    ...
    keywords: ['Kafka', 'Partition', 'Message Ordering', '분산 처리']
  }
];
...
fs.writeFileSync(outputPath, JSON.stringify(completeQuestions, null, 2));

console.log(`Saved ${completeQuestions.length} complete questions with answers`);
